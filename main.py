    ...
    with Progress() as progress:
        ...
        for i, chapter in enumerate(book_data['chapters'], start=1):
            ...
            try:
                ...
                if author_name: audio.add(TPE1(encoding=3, text=author_name))
                if narrator_name: audio.add(TPE2(encoding=3, text=narrator_name))
                if year_text: audio.add(TDRC(encoding=3, text=year_text))
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if scraper:
            break
        console.print("[red]Error: Unsupported website. Please enter a valid URL from a supported site.[/red]")
    ...
    if console.input("[yellow]Do you want to change any of these details? (y/n): [/yellow]").lower().strip() in ('y', 'yes'):
        console.print("\n[cyan]Enter new details. Press Enter to keep the current value.[/cyan]")
        book_data['title'] = console.input(f"Title [{book_data.get('title', '')}]: ").strip() or book_data.get('title')
        book_data['author'] = console.input(f"Author [{book_data.get('author', '')}]: ").strip() or book_data.get('author')
        book_data['narrator'] = console.input(f"Narrator [{book_data.get('narrator', '')}]: ").strip() or book_data.get('narrator')
        book_data['year'] = console.input(f"Year [{book_data.get('year', '')}]: ").strip() or book_data.get('year')
        book_data['cover_url'] = console.input(f"Cover URL [{book_data.get('cover_url', '')}]: ").strip() or book_data.get('cover_url')
    ...
    if book_data.get('cover_url'):
        ...
        try:
            ...
            book_data['mime_type'] = "image/jpeg" if book_data['cover_url'].lower().endswith(('.jpg', '.jpeg')) else "image/png"
