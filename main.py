...
def download_and_tag_audiobook(book_url, cover_url, author_name, year_text, narrator_name):
    """
    ...
    display_title = (sanitized_title[:48] + '..') if len(sanitized_title) > 50 else sanitized_title
    ...
    print(f"| {'Author':<13} | {(author_name if author_name else 'Not Provided'):<42} |")
    print(f"| {'Narrator':<13} | {(narrator_name if narrator_name else 'Not Provided'):<42} |")
...
if __name__ == "__main__":
    ...
    input_cover_url = input("Enter the direct URL for the cover image (or press Enter to skip): ").strip()
    input_author_name = input("Enter the author's name (or press Enter to skip): ").strip()
    while True:
        input_year_text = input("Enter the publication year (or press Enter to skip): ").strip()
        if not input_year_text or (input_year_text.isdigit() and 1000 <= int(input_year_text) <= 2169):
            break
        print("Error: Please enter a valid year (e.g., 2069) or leave blank to skip.")
    input_narrator_name = input("Enter the narrator's name (or press Enter to skip): ").strip()
